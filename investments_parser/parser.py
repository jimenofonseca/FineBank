...
def main():
    ...
    print("success processing investments!")


    # get location of output file
    DIRECTORY_LIABILITIES = directory["DIRECTORY_LIABILITIES"]
    DATA_PROCESSED_LIABILITIES = directory["DATA_PROCESSED_LIABILITIES"]
    DATA_PROCESSED_LIABILITIES_METADATA = directory["DATA_PROCESSED_LIABILITIES_METADATA"]

    # process data
    # get list of all excel files in there (it avoids hidden files):
    from os import listdir
    from os.path import isfile, join
    onlyfiles = [f for f in listdir(DIRECTORY_LIABILITIES) if
                 (isfile(join(DIRECTORY_LIABILITIES, f)) and f.endswith('.xlsx') and not f.startswith('~'))]
    ...
    registry = pd.DataFrame()  # here we collecte th
    for file in onlyfiles:
        file_path = os.path.join(DIRECTORY_LIABILITIES, file)
        xls = pd.ExcelFile(file_path)
        accounts = xls.sheet_names
        accounts.remove('VAL')  # so it does not provide the hidden sheet.
        print('Reading the next accounts %s from file %s' % (accounts, file))
    ...
    # save processed data to disk
    registry.to_csv(DATA_PROCESSED_LIABILITIES)

    # save the accounts, their type and their currency as a data_frame
    registry_metadata = pd.DataFrame({"ACCOUNT": registry_accounts,
                                       "TYPE": registry_type_per_account,
                                       "CURRENCY": registry_currency_per_account})
    registry_metadata.to_csv(DATA_PROCESSED_LIABILITIES_METADATA)
    print("success processing liabilities!")
